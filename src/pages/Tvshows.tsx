...
import { getGenres, searchMovie, searchShows } from "@/helpers";
import type { Genre, Show } from "@/types";
import { useNavigate } from "react-router-dom";
import Tvshowitem from "@/components/ui/TvshowItem";
      ...
      <div className="customContainer">
        <div className="pt-[120px]">
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-4 pb-4">
            {showList &&
              showList.length > 0 &&
              showList.map((movieItem: Show) => (
                <Tvshowitem key={movieItem.id} {...movieItem} />
              ))}
          </div>
        </div>
      </div>
